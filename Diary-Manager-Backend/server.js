...
app.get("/notes/:id",authenticate, async function (req, res) {
    try {
        ...
        let resUser = await db.collection("notes").find({email:req.params.id}).toArray();
...
app.post("/dairy", authenticate, async function (req, res) {
    try {
        //Step 1 : Create a  Connection between Node.js and MongoDB
        const connection = await mongoClient.connect(URL);
...
app.get("/dairy/:id", authenticate, async function (req, res) {
    try {
        //Step 1 : Create a  Connection between Node.js and MongoDB
        const connection = await mongoClient.connect(URL);
        ...
        //Step 3 : Select the Collection
        //Step 4 : Do the Operation (Create, Update, Read, Delete)
        let user = await db.collection("notes").findOne({ _id: mongodb.ObjectId(req.params.id) });
...
app.put("/dairy/:id", authenticate, async function (req, res) {
    ...
    try {
        ...
        //Step 3 : Select the Collection
        //Step 4 : Do the Operation (Create, Update, Read, Delete)
        let user = await db.collection("notes").findOneAndUpdate({ _id: mongodb.ObjectId(req.params.id) }, { $set: req.body })
...
app.delete("/dairy/:id", authenticate, async function (req, res) {
    try {
        //Step 1 : Create a  Connection between Node.js and MongoDB
        const connection = await mongoClient.connect(URL);
        ...
        //Step 3 : Select the Collection
        //Step 4 : Do the Operation (Create, Update, Read, Delete)
        let user = await db.collection("notes").findOneAndDelete({ _id: mongodb.ObjectId(req.params.id) })
